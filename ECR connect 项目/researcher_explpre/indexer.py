...
def xml_parser(file_path, attri_list):
    ...
    for doc in root.findall('Researcher'):

        id = doc.find('ResearcherNo').text
        title = ''
        main_text = ''
        ...
        for attr in attri_list:
            ...
            if attr == 'Describle':
                interest = text_process(val, stem_flag=0)
        id_text_dict[id] = interest
    return id_text_dict
...
def indexing(id_text_dict):
    ...
    # indexing step 1: term sequence -> {term:id}
...
def form_term_id_tfidf_bm25(id_text_dict, indexed_dict):
    ...
    for term in term_id_tfidf_bm25_dict.keys():
        ...
        for key in term_id_tfidf_bm25_dict[term].keys():
            ...
            bm25 = (tf * (k1 + 1)) / (tf + k1 * (1 - b + b * (dl_dict[key] / avg_dl))) * np.log10(
                1 + (N - df) + 0.5 / (df + 0.5))
            term_id_tfidf_bm25_dict[term][key] = (float(round(tfidf, 4)), float(round(bm25, 4)))
